...
subCategoryRouter.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (!subCategorys) {
      return res
        .status(404)
        .json({ msg: "there is no subcategory with this id" });
    }
...
subCategoryRouter.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (!subCategory) {
      res
        .status(404)
        .json({ msg: "The category with the given ID not exists" });
    }
...
subCategoryRouter.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    res
      .status(200)
      .json({ subCategorys, msg: "subCategory is created successfully" });
  })
);
...
subCategoryRouter.patch(
  ...
  asyncWrapper(async (req, res) => {
    ...
    res
      .status(200)
      .json({ subCategory, msg: "subCategory is updated successfully" });
  })
);
...
subCategoryRouter.delete(
  ...
  asyncWrapper(async (req, res) => {
    ...
    res
      .status(200)
      .json({ subCategory, msg: "subCategory is deleted successfully" });
  })
);

//delete products and images with subCategory id
subCategoryRouter.delete(
  "/delete/:id",
  asyncWrapper(async (req, res) => {
    const { id: subCategoryID } = req.params;
